...
export class TableRegistry {
  ...
  create(tableId: string, smallBlind: number, bigBlind: number, maxPlayers = 9): PokerTable {
    if (this.tables.has(tableId)) throw new Error('Table already exists');
    const table = new PokerTable(tableId, smallBlind, bigBlind, maxPlayers);
