...
export class PokerTable {
  ...
  constructor(tableId: string, smallBlind: number, bigBlind: number, maxPlayers = 9) {
    ...
    this.state = {
      tableId, handId: null, maxPlayers, players: [], dealerButton: -1,
      smallBlind, bigBlind, communityCards: [], street: 'WAITING',
      currentPlayerId: null, currentBet: 0, minRaise: bigBlind,
      pots: [], actionDeadline: null, winners: [], events: [],
    };
  }
  ...
  startHand(): void {
    ...
    for (const player of this.state.players) {
      player.holeCards = []; player.currentBet = 0; player.totalContribution = 0; player.hasActed = false; player.canRaise = true; player.status = player.stack > 0 ? 'ACTIVE' : 'OUT';
    }
  ...
  act(action: Action): void {
    ...
    switch (action.type) {
      ...
      case 'CALL': {
        ...
        this.putChips(player, Math.min(toCall, player.stack)); player.hasActed = true; break;
      }
      case 'BET':
  ...
  private awardUncontested(): void {
    ...
    winner.stack += amount; this.state.winners = [{ playerId: winner.playerId, amount, category: 'UNCONTESTED' }];
    this.state.street = 'SETTLEMENT'; this.state.currentPlayerId = null; this.recordEvent({ type: 'POT_SETTLED', playerId: winner.playerId, amount });
